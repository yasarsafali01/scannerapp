...
export default function MultiScanView({ onDone, initialFiles }) {
  ...
  async function rotateCurrentBy(degrees) {
    ...
    try {
      ...
      window.alert(t("edit.rotateFailed"));
    } finally {
      setRotating(false);
    }
  }
  ...
  function startQueue(files) {
    ...
    if (remaining <= 0) {
      window.alert(t("multiscan.pageLimitMessage", MAX_PAGES));
      return;
    }
    let batch = files;
    if (files.length > remaining) {
      batch = files.slice(0, remaining);
      window.alert(t("multiscan.pageLimitMessagePartial", MAX_PAGES, files.length, remaining));
    }
      ...
      <section className="card result">
        <div className="card-header">
          <div className="card-header-title">
            ...
            <h2>{t("titles.result")} ({t("result.pagesCount", result.pageCount)})</h2>
          </div>
          <button className="secondary-btn" onClick={onDone} type="button">
            {t("multiscan.backToHome")}
          </button>
        </div>
        ...
        {result.text !== null && (
          <div className="ocr-text">
            ...
            <pre>{result.text || t("result.noTextFound")}</pre>
          </div>
        )}
      </section>
    );
  ...
  if (current) {
    return (
      <section className="card">
        ...
        <p className="hint-text">{t("edit.dragCorners")}</p>
        {queue.length > 0 && <p className="hint-text">{t("multiscan.queueRemaining", queue.length)}</p>}

        <p className="section-title">{t("edit.rotateSection")}</p>
        ...
        <p className="section-title">{t("edit.adjustSection")}</p>
    ...
    <section className="card">
      ...
      {pages.length === 0 ? (
        <div className="empty-state" style={{ marginTop: 8 }}>
          ...
          <p>{t("multiscan.noPagesYet")}</p>
          <p className="hint-text">{t("multiscan.addPagesHint")}</p>
        </div>
      ) : (
        <div className="page-strip">
          {pages.map((p, i) => (
            <div className="page-thumb" key={i}>
              <img src={p.previewUrl} alt={t("multiscan.pageLabel", i + 1)} />
              <span>{t("multiscan.pageLabel", i + 1)}</span>
      ...
      {pages.length > 0 && (
        <div className="controls">
          <p className="section-title" style={{ margin: 0 }}>{t("edit.modeSection")}</p>
          <div className="modes">
            {MODES.map((m) => (
              <button
                key={m.value}
          ...
          <button className="scan-btn" onClick={finish} disabled={loading} type="button">
            {loading ? t("edit.scanning") : t("multiscan.finishScan", pages.length)}
          </button>
          {loading &&
            (uploadProgress < 1 ? (
              <ProgressBar progress={uploadProgress} label={`${t("edit.uploading")} %${Math.round(uploadProgress * 100)}`} />
            ) : (
              <ProgressBar progress={1} indeterminate label={t("multiscan.processingPages")} />
            ))}
        </div>
      )}
